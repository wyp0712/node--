...
function updateQuestionCount_college() {
    ...
    async function all() {
        ...
        for (let i = 0; i < data.length; i++) {
            const item = data[i];
            let sql2 = `select questions from course where parent_id='${item.id}'`
            let num_arr = await readHandle(sql2)

            let count = 0
            num_arr.forEach(i => {
                count += i.questions
            });
            let sql3 = `update college set questions=${count} where id='${item.id}'`

            await sqlHandle(sql3)
        }
    }
    all().then(() => {
        // console.log("更新学院的题目数量成功")
    }).catch((err) => {
        console.log(err)
    })
}
